...
import {validateWorkbookArchive} from '@/lib/calendar-import'
...
export async function readTabularFile(file:File,limits={rows:5000,columns:40}){
  ...
  if(/\.xlsx$/i.test(file.name)){validateWorkbookArchive(bytes);await workbook.xlsx.load(bytes as unknown as Parameters<typeof workbook.xlsx.load>[0])}
...
export function parseImportDate(value:string,row:number){
  ...
  if(!Number.isFinite(date.getTime()))throw new Error(`Invalid date on row ${row}`)
  return date
}
