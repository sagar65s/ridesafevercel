...
import { registrationAccessToken, registrationService } from '@/lib/services/registrationService'

export const dynamic = 'force-dynamic'

export async function GET(req: NextRequest, { params }: { params: Promise<{ billId: string }> }) {
  try {
    if (!billplzAdapter.isMockEnabled()) return NextResponse.json({ error: 'Mock payments are disabled' }, { status: 404 })
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ billId: string }> }) {
  try {
    ...
    if (pending.status === 'PAID') {
      return NextResponse.json({ success: true, status: 'PAID', regId: pending.id, token: registrationAccessToken(pending.id) })
    }
    ...
    if (outcome === 'paid') {
      ...
      return NextResponse.json({ success: true, status: 'FAILED', regId: pending.id, token: registrationAccessToken(pending.id) })
    }
  } catch (error) {
