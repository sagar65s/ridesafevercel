...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    for(const row of rows){
      ...
      const student=students.find(item=>(studentCode?item.studentCode?.toLowerCase()===studentCode.toLowerCase():item.name.toLowerCase()===studentText.toLowerCase())&&item.routeId===trip.routeId&&item.busId===trip.busId)
    ...
    return NextResponse.json({created:created.count,skipped:errors.length,duplicates:records.length-created.count,errors:errors.slice(0,50)})
