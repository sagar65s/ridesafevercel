...
export async function GET() {
  try {
    ...
    if (!user || !['ADMIN', 'SCHOOL_ADMIN', 'SUPER_ADMIN'].includes(user.role)) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
    const actor = await getCurrentUser()
    if (!actor) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    const organizationId = actor.role === 'SUPER_ADMIN' ? undefined : actor.organizationId || '__none__'
    ...
    const [students, trips, notifications, attendances] = await Promise.all([
      prisma.student.findMany({ where: organizationId ? { organizationId } : {}, select: { id: true, status: true, isSelfPickup: true } }),
      prisma.trip.findMany({
        where: { date: { gte: sevenDaysAgo }, ...(organizationId ? { route: { organizationId } } : {}) },
        select: { id: true, status: true, date: true }
      }),
      prisma.notification.findMany({
        where: { createdAt: { gte: sevenDaysAgo }, ...(organizationId ? { user: { organizationId } } : {}) },
        select: { id: true, type: true, createdAt: true }
      }),
      prisma.attendance.findMany({
        where: { timestamp: { gte: sevenDaysAgo }, ...(organizationId ? { trip: { route: { organizationId } } } : {}) },
