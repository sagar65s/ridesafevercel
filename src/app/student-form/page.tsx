...
const defaultForm = {
  ...
  parentContact1: '', parentContact2: '', selfPickupSession: '', organizationId: '',
}
...
function PaymentConfirmation({ regId, token }: { regId: string; token: string }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      tries += 1
      try {
        const res = await fetch(`/api/public/register/status?regId=${encodeURIComponent(regId)}&token=${encodeURIComponent(token)}`)
...
function RegistrationForm() {
 const {tx:translateUi}=useLocaleText()
  ...
  useEffect(() => {
    fetch('/api/public/organizations').then(r => r.ok ? r.json() : { organizations:[] }).then(data => {
      const items = data.organizations || []
      setOrganizations(items)
      if (items.length === 1) setForm(current => ({ ...current, organizationId:items[0].id }))
    }).catch(() => {})
  }, [])
    ...
    <div style={{ background: HC.bgSoft, border: `1px solid ${HC.line}`, borderRadius: HC.r, padding: '32px 28px' }}>
      ...
      <form onSubmit={handleSubmit}>
        {organizations.length > 1 && (
          <>
            <label style={labelStyle()}>{t('registration.school')} *</label>
            <select className="select-field" style={fieldStyle(false)} required value={form.organizationId} onChange={e => set('organizationId', e.target.value)}>
              <option value="">{t('registration.selectSchool')}</option>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap', justifyContent:'flex-end' }}>
            ...
            <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 6, color: HC.text3, textDecoration: 'none', fontSize: 13, fontWeight: 600 }}>
              <ArrowLeft size={14} /> {t('registration.backLogin')}
            </Link>
          </div>
        </div>

        {regId && token ? <PaymentConfirmation regId={regId} token={token} /> : <RegistrationForm />}
      </div>
    </div>
  )
}
