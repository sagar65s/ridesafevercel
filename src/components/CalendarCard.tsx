'use client'
import { useTranslation, TranslatedText } from '@/i18n/provider'
...
export default function CalendarCard() {
  ...
  const upcomingEvents = events
    .filter(e => new Date(e.endDate || e.startDate) >= new Date(new Date().setHours(0,0,0,0)))
  ...
  return (
    <div className="glass-panel" style={{ padding: '1.5rem', height: '100%', display:'flex', flexDirection:'column' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'1.25rem' }}>
        <div>
          <h3 style={{ margin:0, fontSize:'1.1rem', display:'flex', alignItems:'center', gap:8 }}>
            <Calendar size={20} color="var(--primary)"/><TranslatedText text={" School Calendar "}/></h3>
          <p style={{ margin:'4px 0 0 0', fontSize:'0.8rem', color:'var(--text-muted)' }}><TranslatedText text={"Upcoming key dates"}/></p>
        </div>
      </div>

      <div style={{ display:'flex', flexDirection:'column', gap:'0.75rem', flex:1 }}>
        {upcomingEvents.length === 0 ? (
          <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.85rem', color:'var(--text-dim)', textAlign:'center' }}><TranslatedText text={" No upcoming events scheduled. "}/></div>
        ) : (
            ...
            >
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:'0.7rem', color:'var(--text-muted)' }}>
                  <TranslatedText text={event.type.replace('_', ' ')}/>
                </div>
              </div>
            </motion.div>
          ))
      ...
      {events.length > 3 && (
        ...
        }}>{tx(expanded ? 'Show less' : 'View full calendar')}<ChevronRight size={16}/>
        </button>
      )}
    </div>
  )
